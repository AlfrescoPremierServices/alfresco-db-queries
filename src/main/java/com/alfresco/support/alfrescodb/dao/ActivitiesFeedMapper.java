...
public interface ActivitiesFeedMapper {
    ...
    @Select("select count(*) as occurrences, substr(post_date, 1, 11) as postDate, site_network as siteNetwork, activity_type as activityType\n" +
            "from alf_activity_feed\n" +
            "where feed_user_id != '@@NULL@@'\n" +
            "and feed_user_id = post_user_id\n" +
            "group by post_date, site_network, activity_type\n")
    ...
    @Select("select count(*) as occurrences, substr(post_date, 1, 11) as postDate, site_network as siteNetwork, feed_user_id as feedUserId\n" +
            "from alf_activity_feed\n" +
            "where feed_user_id != '@@NULL@@'\n" +
            "and feed_user_id = post_user_id\n" +
            "group by post_date, site_network, feed_user_id")
    ...
    @Select("select count(*) as occurrences, substr(post_date, 1, 11) as postDate, site_network as siteNetwork, app_tool as appTool\n" +
            "from alf_activity_feed\n" +
            "where feed_user_id != '@@NULL@@'\n" +
            "and feed_user_id = post_user_id\n" +
            "group by post_date, site_network, app_tool")
