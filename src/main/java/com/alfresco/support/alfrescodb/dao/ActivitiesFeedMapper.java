...
public interface ActivitiesFeedMapper {
    ...
    @Select("select count(*) as occurrences, substring(post_date, 1, 10) as postDate, site_network as siteNetwork, activity_type as activityType " +
            "from alf_activity_feed " +
            "where feed_user_id != '@@NULL@@' " +
            "and feed_user_id = post_user_id " +
    ...
    @Select("select count(*) as occurrences, substring(post_date, 1, 10) as postDate, site_network as siteNetwork, feed_user_id as feedUserId " +
            "from alf_activity_feed " +
            "where feed_user_id != '@@NULL@@' " +
            "and feed_user_id = post_user_id " +
    ...
    @Select("select count(*) as occurrences, substring(post_date, 1, 10) as postDate, site_network as siteNetwork, app_tool as appTool " +
            "from alf_activity_feed " +
            "where feed_user_id != '@@NULL@@' " +
            "and feed_user_id = post_user_id " +
    ...
    @Select("select count(*) as occurrences, substring(CONVERT(VARCHAR(10), post_date), 1, 10) as postDate, " +
